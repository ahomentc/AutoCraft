...
This was giving us some problems, which we'll address in the next section. In short, sometimes the entree in the q tables (swtiching or not switching) would go in a race towards negative infinitly, leading to no clear choice. We solved this problem by adding an extra part called "penalizeCompetitor." What this piece of code did was that when one entree in the q table had a positive reward, the other would have a negative reward, and vice versa. The reasoning for this was that the agent would always choose the state with the highest value in the q table. And the rewards aren't always positive, even when switching. So since one was always chosen over the other when it was ahead, sometimes the rewards had no impact because it would converge to the other state's value eventually. This would lead to a race towards negative infinity. By giving the opposite reward to the other state that wasn't chosen, this race was avoided as the rewards would always have an impact on BOTH states.

This was our update_q_table code:
...
We fixed this by implementing an opposite reward to the other action not taken, as described above. This solution wasn't immediatly clear but is much nicer because it is simpler, more elegant, and actually works and converges to swtiching 100% of the time.
